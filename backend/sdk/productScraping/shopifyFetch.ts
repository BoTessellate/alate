...
export async function tryShopifyJSON(
  ...
  try {
    ...
    // Identify size axes from product.options (Shopify always returns
    // these in the live storefront JSON; some test fixtures omit them
    // so we fall back gracefully). Any option whose name contains
    // "size" counts — this catches "Size", "Top Size", "Bottom Size",
    // and the Oshin Sarin two-axis case where a co-ord set carries
    // both axes. When the option list is missing OR no axis name
    // matches, we fall back to `option1` (the legacy single-axis path
    // every brand we'd previously seen relied on).
