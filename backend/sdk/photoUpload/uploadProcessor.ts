...
import { createJobQueue, JobTypes, ProcessCutoutJobData } from '../shared/jobQueue';
...
export async function processPhotoUpload(
  ...
  // Step 4: Enqueue async cutout job (non-blocking)
  let cutoutJobId: string | undefined;
  let cutoutStatus: 'queued' | 'skipped' = 'skipped';

  if (!DEMO_MODE && process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_KEY) {
    try {
      ...
      const job = await jobQueue.enqueue<ProcessCutoutJobData>(
        JobTypes.PROCESS_CUTOUT,
        {
          productId,
      ...
      cutoutJobId = job.id;
      cutoutStatus = 'queued';
      logger.info({ productId, jobId: job.id }, 'Cutout job enqueued');
    } catch (error: any) {
      logger.warn({ productId, error: error.message }, 'Failed to enqueue cutout job');
    }
  }
...
/**
 * Fetch an image URL and return it as a base64-encoded string with its MIME type.
 * Throws on non-2xx HTTP responses so callers can fall through to the next provider.
 */
