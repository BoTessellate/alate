...
-- =============================================================================
-- CUTOUT (BACKGROUND-REMOVED) IMAGES
-- =============================================================================
-- Store URLs to pre-processed product images with backgrounds removed

-- Cutout URL for background-removed product image
ALTER TABLE enriched_products
ADD COLUMN IF NOT EXISTS cutout_url TEXT;

-- Index for faster lookups of products with cutouts
CREATE INDEX IF NOT EXISTS idx_enriched_products_cutout_url ON enriched_products(cutout_url) WHERE cutout_url IS NOT NULL;
